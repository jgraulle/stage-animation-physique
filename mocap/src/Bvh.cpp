...
Bvh::Bvh(const string nomFichier)
{
	...
	// Cr�ation de l'actor render
	//actorRender = new ActorRenderSkin(mocapData, skeletalData, skinData);

    // Une fois que toutes les donn�es sont charg�es
    mocapManipulator = new MocapManipulator(mocapData, skeletalData);
...
//////////////////////////////////////////////////
// PROC:   Bvh::canWalk()
// DOES:   Indique si le personnage peut marcher (si les 2 jambes sont nomm�es en fait)
//////////////////////////////////////////////////
bool Bvh::canWalk() {
...
// DOES:   Change l'echelle de l'animation et du squelette (r�duction, augmentation)
...
void Bvh::fixerTailleSquelette(float valeur) {
	...
	for (int numeroFrame=0; numeroFrame<getNbFrame(); numeroFrame++) {
		...
		for (SkeletalData::JointListIndexedByName::iterator it = listeJoints.begin(); it != listeJoints.end(); it++) {
            ...
            for (int k =1; k<3; k++){
                // La mocapData � retailler:
                // Les donn�es de l'animation ET la posture de r�f�rence contenue dans le skinData
                MocapData * mocapData;
                switch (k) {
    ...
    // On n'oublie pas de retailler le skin aussi (si pr�sent)
    if (skinData)
...
// DOES:   centre l'animation de d�part en (0,?,0)
//////////////////////////////////////////////////
void Bvh::centrerAnimation()
{
	// D�termine X et Z de la racine
	float Xroot, Zroot;
	...
	// Decale l'animation en cons�quence
...
//			en d�pit des d�placements relatifs
//////////////////////////////////////////////////
void Bvh::fixerAnimation()
{
	...
	// Calcul de la droite de r�gression (m�thodes des moindres carr�s)
	...
	for (int f=0; f<getNbFrame(); f++) {
        ...
        // On calcule la distance du point courant � la droite de regression
...
// DOES:   Calcule la vitesse de d�placement de l'animation de marche (si cel� en est une)
...
{
	// D�tection de la phase de support
...
void Bvh::fixerGlissementPied(int numeroFrame, float xPers, float yPers, float direction, string  nomHautJambe1, string  nomHautJambe2) {
	...
	// Si aucun support n'a �t� trouv�, on ne fait rien
...
void Bvh::detecterSupportPied(string  nomPremierPied, string  nomDeuxiemePied) {
	...
	// La frame de d�but du support du pied
	...
	for (int f=0; f<getNbFrame(); f++) {
		...
		if (frameDebutSupport == -1) {
			...
			// Selon la situation, on teste si un des 2 pieds se rel�ve
			if ((posPied->getY() - minYpied1 > epsilonAltitude)
    ...
    */
    ...
    // On consid�re que le mouvement a �t� rendu cyclique par notre module
    // Donc forc�ment le d�but de la mocap est une phase de support du pied initiateur
    // Il suffit alors de d�terminer quel est le pied initiateur pour d�terminer l'ordre
    // de d�but/fin support
    // En effet frameDebutSupport est toujours le d�but du support du pied1 (qu'il soit gauche ou droite)


    const float epsilonDeplacement = 0.2;
    const float epsilonAltitude = 0.2;

    // On d�termine alors � quel moment le premier pied fini sa phase
    // (pas forc�ment en milieu de l'animation cyclique pour une marche
    ...
    // On fixe alors le d�but et la fin de la phase de support du pied1
    ...
    */
    ...
    // Bon finalement je zappe, on prend la moiti� et c tout
    this->frameDebutSupport = 1;
...
void Bvh::rendreCyclique(string  nomPremierPied, string  nomDeuxiemePied) {
	...
	// Etape 1: d�tecter un cycle de marche (pose d'un pied, puis du second)
	...
	int numeroFrame = 1; // On part � la frame 1 pour pouvoir calculer la vitesse
	bool pied1Last = true; // On considere � tort ou a raison que le pied1 touche le sol en premier
	int tabFrameCle[4]; // Tableau contenant les indexs des frames cl�s
	int indexTabFrameCle = 0; // Variable d'index du tableau ci-dessus

	// Pour les 4 frames cl�s que l'on veut trouver:
	for (int k=0; k<4; k++) {
		...
		// On attends qu'un des deux pieds se rel�ve
		while (numeroFrame < getNbFrame()) {
			// Recupere la position du pied
			Point3D * posPied = getPositionJointByNameInFrame(numeroFrame, nomPremierPied, true);
			Point3D * posPiedOld = getPositionJointByNameInFrame(numeroFrame-1, nomPremierPied, true);
	...
	// A ce stade, notre cycle de marche non corrig� se situe entre les frames
	// tabFrameCle[1] et tabFrameCle[3]. La frame tabFrameCle[2] correspond �
	// la pose du pied interm�diaire (autre que celui de d�part)
	// On ignore volontairement tabFrameCle[0] car la premi�re pose du pied
	// peut correspondre au passage d'un �tat statique � la marche
	// ce qui donne un mauvais r�sultat pour produire un cycle


	// Etape 2: Extraire le cycle non corrig� de l'animation initiale
	...
	// Etape 3: m�lange entre le d�but et la fin de l'animation pour assurer
	// la transition cyclique

	// On commence le m�lange sur une portion �gale a 40% de la fin de l'animation
	int frameDebutMelange = (int)(0.6 * getNbFrame());

	// M�lange degressif de la fin vers frameDebutMelange
	for (numeroFrame=getNbFrame()-1; numeroFrame>=frameDebutMelange; numeroFrame--) {
		...
		// Pour chaque param�tre de la frame courante
		/*
		for (int i=0; i<getNchannels(); i++) {
			//int indexCourant = numeroFrame*nchannels+i;
			// Avant d'effectuer le m�lange, on v�rifie qu'il ne s'agit
			// pas de 2 angles <-90� et >90� dont le m�lange donnerait une mauvaise valeur
			// Exemple: -179 + 179� donnerait 0� alors que le r�sultat doit �tre 180�
			...
			// On effectue le m�lange avec la frame qui la succede
		...
		*/
		...
		for (SkeletalData::JointListIndexedByName::iterator it = listeJoints.begin(); it != listeJoints.end(); it++) {
            ...
            // - M�lange des 3 translations si elle existent
            if (jointCourant->isTransJoint()) {
            ...
            for (int k=0; k<3; k++) {
    			...
    			// On effectue le m�lange avec la frame qui suit
...
void Bvh::turnAroundY(float angle) {
	// On fixe le d�part de l'animation en (0,?,0);
	centrerAnimation();

	// Correction de la trajectoire
	for (int f=0; f<getNbFrame(); f++) {

		// Etape 0: On recalcule les 3 angles d'euler de la racine en ayant effectu�
		...
		// pour r�cuperer les coordonn�es du point du terminal
...
*/
		...
		// Translation pour d�part en (0,0)
		//Xroot -= XrootInitO;
		//Zroot -= ZrootInitO;
		// Rotation des coordonn�es de la racine dans le plan XZ (axe Y)
...
void Bvh::alignerAnimationSensDeMarche()
{
	...
	// Recherche de l'angle de la trajectoire sur la totalit� du chemin

	// On recup�re la positon de la racine en d�but et en fin de parcours
	...
	// On calcule la position en d�but et en fin de parcours sur la droite de r�gression
	float XrootInit = XrootInitO;
	//float ZrootInit = b0 + b1*XrootInit;
	float XrootTerm = XrootTermO;
	//float ZrootTerm = b0 + b1*XrootTerm;

	// Angle de la trajectoire par rapport � l'axe X
	...
	// On l�ve l'ind�termination de la valeur renvoy� par la fonction tangente inverse
	// en v�rifiant que la progression sur l'axe X se fera dans le sens croissant
...
// DOES:   Renvoie la liste complete des positions des terminaux � partir d'un joint de r�f�rence
//////////////////////////////////////////////////
list<Point3D*> * Bvh::getPositionTerminaux(float* frameData, Joint * referenceJoint) {

	glPushMatrix();

	// Pour sauver les r�sultats
	...
	if (referenceJoint->isTerminal()) {
		...
		// On translate sur la fin r�elle du terminal
		...
		// On ajoute la position du terminal � la liste
...
// DOES:   renvoie la position d'un joint par son nom � partir d'un joint de reference
//////////////////////////////////////////////////
...
//Point3D * Bvh::getPositionJointByName(int numeroFrame, string nomJoint, bool getEndSite, float *matRot) {
	...
	if (referenceJoint->getName() == nomJoint) {
		...
		// Sauvegarde la matrice de rotation trouv�e si n�cessaire
		if (matRot !=  NULL) {
...
string Bvh::getEndJointName(string nomJoint) {
	...
	// Ici on consid�re que la chaine est simple (sans d�doublement)
...
// DOES:   Renvoie la chaine entier vers un terminal � partir d'un joint
//////////////////////////////////////////////////
...
// DOES:   Met � jour / construit la liste des joint "listeJoints"
//////////////////////////////////////////////////
/*
...
Bvh::JointListIndexedByName Bvh::majListeJoints() {
    ...
    // Commence la mise � jour par la racine
...
// DOES:   Utilis�e pour la r�cursivit� de la m�thode publique getListAllJoint()
//////////////////////////////////////////////////
map<string, void*> * Bvh::getListAllJoint(Joint * referenceJoint) {
	// Cr�ation de la liste r�sultat
	...
	for (list<Joint*>::iterator it = listeCourante->begin(); it != listeCourante->end(); it++) {
		...
		// et recup�ration de la liste des noeud
		map<string, void*> * listeNoeud = getListAllJoint(unenfant);

		// Insertion de cette branche dans l'arbre r�sultat
...
// DOES:   Renvoie une stl::map d'association cl�-valeur contenant l'arbre du squelette du personnage
...
map<string, void*> * Bvh::getListAllJoint() {
	...
	// On lance l'appel r�cursif sur la liste des joints
...
// DOES:   Ik par ccd sur une chaine simple (pas de d�doublement)
//		   Fournir le num�ro de la frame � traiter, le nom du joint d�signant le d�but de la chaine,
//		   la position voulue pour la fin de cette chaine, et le nombre d'it�ration maximum
...
void Bvh::appliquerIkCcd(int numeroFrame, list<Joint*> * chaine, Point3D positionVoulue, int nbIterationsMax) {
	...
	// On r�cup�re le terminal de la chaine
	...
	// On repete la passe d'optimisation autant de fois que n�cessaire
	for (int precision=0; precision<nbIterationsMax; precision++) {
		...
		for(list<Joint*>::iterator it = --chaine->end(); it != chaine->begin(); it--) { // de la base vers le terminal
			...
			// Calcul de la positon du terminal et recup�ration de la matrice de rotation
			// Pour plus de pr�cision il faut d�placer ce bloc dans la boucle de parcours de la chaine (fait)
			...
			// Calcul de Pic: vecteur de distance entre d�part et terminal
			Point3D Pic = Point3D((posTerm->getX() - posDepart->getX()),
									(posTerm->getY() - posDepart->getY()),
									(posTerm->getZ() - posDepart->getZ()));

			// Calcul de Pid: vecteur de distance entre d�part et la position voulue
			...
			// Mise � jour des param�tres des coefficients k
			...
			// On applique la matrice de vue du terminal � X Y Z
			// puis on soustraint la position du terminal, pour obtenir le nouveau repere X' Y' Z' bien orient�
			// selon les 3 degr�s de libert� du joint
			...
			// Calcul des 3 axes (on applique la matrice de vue de d�part � X Y Z)
			// puis on soustraint la position de d�part, pour obtenir le nouveau repere X' Y' Z' bien orient�
			// selon les 3 degr�s de libert� du joint
			...
			// K2
			// NOTE: le calcul de K2 ne d�pend pas de l'axe courant (voir dans la boucle suivante)
			float k2 = 0;
			for (int j = 0; j<3; j++) {
			...
			for (int axe = 0; axe <3; axe++) {
				...
				//TETA
				//Le deplacement de l'angle optimis�
				float teta;
				if (k2-k1 != 0) {
...
/* Ancienne version pas du tout optimis�e */
/*
void Bvh::appliquerIkCcd(int numeroFrame, string nomJoint, Point3D positionVoulue, int nbIterationsMax) {
	// Constantes de l'algortihme
	...
	for (int precision=0; precision<nbIterationsMax; precision++) {
		...
		for(list<Joint*>::iterator it = chaine->begin(); it != chaine->end(); it++) {
			...
			for (int axe = 1; axe <=3; axe++) {
				...
				// Calcul des 3 axes (on applique la matrice de vue de d�part a X Y Z
				// puis on soustraint la position de d�part, pour obtenir le nouveau repere X' Y' Z' bien orient�
				// selon les 3 degr�s de libert� du joint
				Point3D axed1 = Point3D(1,0,0);
				axed1.multVecteurMatrice(matRotDepart);
...
//		   Aucun retimage effectu�
//////////////////////////////////////////////////
Bvh::Bvh * Bvh::interpoler(Bvh* bvh1, Bvh * bvh2, float poids) {

    // D�claration de l'animation finale
    Bvh * bvhResult;
    ...
    // Calcul et stockage de la longueur totale de la mocap r�sultante
    ...
    // RETIMING + Interp
    // (bon pour l'instant on consid�re que la dur�e des frames est la m�me des 2 cot�s)
	...
	for (SkeletalData::JointListIndexedByName::iterator it = listeJointsBvh1.begin(); it != listeJointsBvh1.end(); it++) {
        // On r�cupere le joint courant
        Joint * jointCourantBvh1 = it->second;

        // On r�cupere le joint associ� dans l'anim2
        Joint * jointCourantBvh2 = bvh2->getSkeletalData()->getJointByName(jointCourantBvh1->getName());
        if (jointCourantBvh2 == NULL) return NULL;

        // On r�cupere le joint concern� (le joint r�sultat)
        // On n'effectue aucune v�rification de pr�sence du joint, car l'animation �tant produite � partir
        // d'une des deux anims � interpoler, le joint existe FORCEMENT
        ...
        // Pour chaque frame de l'animation r�sultante
        for (int numeroFrame=0; numeroFrame<longueurTotale; numeroFrame++) {

            // On r�cupere les donn�es de l'anim1 et 2 en respectant les poids du m�lange
            ...
            // On effectue le m�lange
            MocapData::JointData jointDataResult;

            // M�lange des rotations
            float rotXYZ1[] = {0, 0, 0};
            float rotXYZ2[] = {0, 0, 0};
...
// DOES:   G�nere une mocap diff�rente (en construction, pour tests seulement)
//////////////////////////////////////////////////
...
void Bvh::changerPremierPiedSupport() {
    // On d�tecte les phase de support de chaque pied (si ce n'est pas d�j� fait)
    detecterSupportPied(jambeDroite, jambeGauche);

    // L'id�e est de d�placer l'ensemble des frames de la premiere phase de support vers la phase de retour
    // et inversement

    // Pour chaque joint de la frame
	SkeletalData::JointListIndexedByName listeJoints =  skeletalData->getListeJointsByName();
	for (SkeletalData::JointListIndexedByName::iterator it = listeJoints.begin(); it != listeJoints.end(); it++) {
        // R�cup�ration du joint courant
        Joint * jointCourant = it->second;
        ...
        // Tant que l'on n'a pas d�plac� autant d'�l�ment (de jointData pour le joint courant) que le nombre de frames
    	while (totalElementDeplace < getNbFrame()) {

            // jointDataNew est l'�l�ment en cours de d�placement
            // Au d�part de l'algorithme, il s'agit du premier �l�ment (situ� � dernierIndexDeplace)
            ...
            // Tant que l'on n'a pas fait un tour complet, c'est � dire que l'on n'est pas revenu
            // � l'index de d�part
            int sauvLastIndex=dernierIndexDeplace;
            do {

                // On r�cupere les donn�es � l'index ou l'on va �crire
                MocapData::JointData jointDataTemp = mocapData->getJointData(dernierIndexDeplace, jointCourant);

                // On y place les donn�es du joint sauvegard� dans jointDataNew
                mocapData->setJointData(jointDataNew,
                                        dernierIndexDeplace,
                                        jointCourant);

                // Finalement on place les donn�es sauvegard�es � l'index qui vient d'�tre �cras�
                // dans jointDataNew (et on boucle l'algo)
                jointDataNew = jointDataTemp;

                // On totaliste un �l�ment de plus d�plac� pour ce joint
                totalElementDeplace++;
            ...
            // On passe � l'index suivant (si utile, si encore des �l�ments � d�placer)
    ...
    // On final, on �change aussi les marqueurs des joints des jambes
...
BvhGlissementPied::BvhGlissementPied(Bvh * bvh, string nomHautJambe) {
	...
	// Cr�ation et sauvegarde des chaines repr�sentant les jambes
...
//		   Il faut donner la position du personnage, sa direction, le nom de la partie sup�rieure de la jambe
//		   Ainsi que les frames de d�but et de fin de support du pied, et le num�ro de la frame � traiter
//////////////////////////////////////////////////
void BvhGlissementPied::fixerGlissementPied(int numeroFrame, int frameDebutSupport, int frameFinSupport, float xPers, float yPers, float direction){
	...
	// Test de faisabilit� (par exemple, si le mouvement n'est pas de la marche)
	if (frameDebutSupport==-1 ||frameFinSupport==-1) return;
	...
	if ((numeroFrame > frameDebutSupport && numeroFrame <= frameFinSupport && frameDebutSupport<frameFinSupport)
		...
		// On va alors calculer la position fixe du pied dans le repere personnage apres que le personnage ait boug�

		// On part de la position du pied dans le repere monde auquel on soustrait les coordon�es du personnage
		Point3D pointPiedFixePers = Point3D(-pointPiedDebutMonde->getX()+xPers,pointPiedDebutMonde->getY(),-pointPiedDebutMonde->getZ()+yPers);

		// On corrige l'�ventuelle rotation du personnage, et on se retrouve dans le repere personnage
		...
		// DEBUG: affichage zone de contact fix�e
        #ifdef DEBUG_IKCCD
		glPushMatrix();
		...
		// On applique la m�thode CCD sur le point d�termin�
		...
		// Force la remise � jour de A et B dans la deuxieme phase
		delete A;
		delete B;
		A = NULL;
		B = NULL;
	...
	// DEUXIEME PHASE: retour coh�rent du pied
	else {
		...
		if ((pointPiedDebutPers && numeroFrame != frameDebutSupport)
			...
			// Sauvegarde de la derni�re frame trait�e
			// Ceci permet de ne pas affecter la trajectoire de retour du pied, m�me
			// si le personnage est en train de bouger (et que donc on apelle la fonction
			// pour mettre � jour la position du pied au sol)
			derniereFrameTraitee = numeroFrame;

			// Si on n'a pas d�tecter la fin du mouvement et les 2 positions des pieds (avant et apres correction)
			if (!A || !B) {
				//detecterSupportPied(getEndJointName(nomHautJambe1),getEndJointName(nomHautJambe2));
				...
				// On place A et B dans le r�pere centr� sur pointPosPied (l� ou on a pos� le pied en d�but de support
				// dans le repere centr� sur le personnage)
			...
			// (aucune correction du retour du pied � faire)
            ...
            // Inversion de signe selon la diff�rence de niveau en d�but et fin de support
			...
			pointCorrige.setX(B->getX());  // On garde le meme �cartement de la jambe qu'� la fin de la phase de pose

			// Replace le point dans le rep�re personnage
			pointCorrige.setZ(pointCorrige.getZ() + pointPiedDebutPers->getZ());
			pointCorrige.setY(pointCorrige.getY() + pointPiedDebutPers->getY());

			// Applique finalement la m�thode CCD
			...
			//DEBUG: affichage du point de contact du pied pr�vu en d�but de support
			#ifdef DEBUG_IKCCD
			glPushMatrix();
